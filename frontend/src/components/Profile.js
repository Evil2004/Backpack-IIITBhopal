...
const Profile = () => {
  ...
  const pendingRequestsToMeCounter = () => {
    let counter = 0;
    (profileData.request_to_me ? profileData.request_to_me : []).map(
      (ele, index) => {
  ...
  return (
    <>
      ...
      <Modal
        show={show}
        onHide={handleClose}
        style={{ backdropFilter: "blur(2px)", overflow: "visible" }}
      >
        <ModalHeader
          closeButton
          style={{
        ...
        >
          <h3>All requests for your books</h3>
        </ModalHeader>
        <Modal.Body
          style={{
            padding: "1.5rem",
        ...
        >
          {pendingRequestsToMe === 0 && (
            <p className="fs-5 fst-italic">No request now</p>
          )}
          {(profileData.request_to_me ? profileData.request_to_me : []).map(
            (ele, index) => {
              if (ele.status === "pending") {
                return (
                  <div
                    key={index}
                    className="border border-light py-2 px-3 rounded mb-4"
                    style={{ background: "lightgrey" }}
                  >
                    <h5>A new request for your "{ele.product.name}" book</h5>
                    <h6>Requesting User Details</h6>
                    <p className="p-0 m-0">
                      Name : {ele.toOwner.first_name} {ele.toOwner.last_name}
                    </p>
                    ...
                    <div className="d-flex justify-content-end">
                      <button
                        ...
                        onClick={() => {
                          handleClose();
                          handleAcceptReject(ele.id, "accepted");
                        }}
                      >
                        Accept
                      </button>
                      <button
                        ...
                        onClick={() => {
                          handleClose();
                          handleAcceptReject(ele.id, "rejected");
                        }}
                      >
                        Decline
                      </button>
      ...
      {!loading && Object.keys(profileData).length !== 0 && (
        <div style={{ ...myStyle, padding: "2rem 2rem", marginTop: "-1rem" }}>
          <div className="row w-100">
            <div
              className={`col-md-4 ${
                window.screen.width > 992 ? "d-flex justify-content-center" : ""
              }`}
              style={{ ...inputMyStyle, borderRadius: "10px" }}
            >
              <div
                style={{ minWidth: "calc(10rem + 10vw)", paddingTop: "1rem" }}
              >
                <img
                  src={profileData.user ? profileData.user.profile_pic : ""}
                  alt=""
                  width={120}
                  height={110}
                  style={{
                ...
                <h4 className="mt-4">
                  {profileData.user
                    ? capitalize(profileData.user.first_name)
                    : ""}{" "}
                  {profileData.user
                    ? capitalize(profileData.user.last_name)
                    : ""}
                </h4>
                <h6 className="mt-2">
                  @{profileData.user ? profileData.user.username : ""}
                </h6>
                ...
                >
                  <li
                    className="list-group-item mt-2 p-0"
                    style={{
                      ...inputMyStyle,
                      border: "none",
                      letterSpacing: ".5px",
                  ...
                  <li
                    className="list-group-item mt-1 p-0 fw-bold"
                    style={{
                      ...inputMyStyle,
                      border: "none",
                      letterSpacing: ".5px",
                  ...
                  >
                    {profileData.user ? profileData.user.college : ""}
                  </li>
                  <li
                  ...
                  >
                    {profileData.user ? profileData.user.batch : ""}
                  </li>
                  <li
                    className="list-group-item mt-3 p-0"
                    style={{
                  ...
                  >
                    {profileData.user ? profileData.user.address : ""}
                  </li>
                  <li
                    className="list-group-item mt-3 p-0"
                    style={{
                  ...
                  <li
                    className="list-group-item mt-1 p-0 fw-bold mb-4"
                    style={{
                      ...inputMyStyle,
                      border: "none",
                      letterSpacing: ".5px",
              ...
              <div>
                ...
                <div>
                  <div
                    ...
                    style={{
                      marginTop: "calc(2rem - 2vw)",
                      ...inputMyStyle,
                      borderRadius: "10px",
                    }}
                  >
                    <h2>Your Books</h2>
                    {profileData.product.length === 0 && (
                      <p className="fs-5 fst-italic">No books show</p>
                    )}
                        ...
                        >
                          ...
                          >
                            ...
                            <h6 className="text-center mt-2">
                              ...
                              <button
                                disabled={ele.available === true ? true : false}
                                style={{
                                  border: `${
                                    ele.available === true
                                      ? `2px solid ${
                              ...
                              <button
                                disabled={
                                  ele.available === false ? true : false
                                }
                                style={{
                                  border: `${
                                    ele.available === false
